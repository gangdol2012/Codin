#!/usr/bin/env node
/*
 * CodeCraft C# restore/apply exact v3 any-context completions from the actual GitHub main state.
 *
 * Actual GitHub main was verified to contain contextual-v2 at generation time, not v3/v4.
 * This script is conservative:
 * - It only touches src/csharp-omnisharp.ts completion code.
 * - It removes only known marker-delimited completion helper blocks.
 * - It does not depend on backup files.
 * - It installs the exact v3 any-context completion helper/methods.
 *
 * Dead leftover code outside known helper markers is allowed; marker-delimited known
 * helper blocks are removed to avoid duplicate const/function declarations.
 */
...
const KNOWN_COMPLETION_HELPER_BLOCKS = [
  [HARDENING_START, HARDENING_END],
  [CONTEXTUAL_V2_START, CONTEXTUAL_V2_END],
  [HELPER_START, HELPER_END],
  ['// CodeCraft C# completion anywhere fix start', '// CodeCraft C# completion anywhere fix end'],
  ['// CodeCraft C# completion-anywhere semantic fix start', '// CodeCraft C# completion-anywhere semantic fix end'],
  ['// CodeCraft C# hybrid completion fix start', '// CodeCraft C# hybrid completion fix end'],
  ['// CodeCraft C# hybrid v2-v3 completion routing fix start', '// CodeCraft C# hybrid v2-v3 completion routing fix end'],
  ['// CodeCraft C# dot-fast hybrid completion fix start', '// CodeCraft C# dot-fast hybrid completion fix end'],
  ['// CodeCraft C# dot-v2 non-dot-v3 completion fix start', '// CodeCraft C# dot-v2 non-dot-v3 completion fix end'],
  ['// CodeCraft C# hybrid dot-fast completion fix start', '// CodeCraft C# hybrid dot-fast completion fix end'],
  ['// CodeCraft C# hybrid fast-dot completion fix start', '// CodeCraft C# hybrid fast-dot completion fix end'],
  ['// CodeCraft C# mixed dot/context completion fix start', '// CodeCraft C# mixed dot/context completion fix end'],
  ['// CodeCraft C# mixed fast-dot completion fix start', '// CodeCraft C# mixed fast-dot completion fix end'],
  ['// CodeCraft C# v2-dot-v3-context completion fix start', '// CodeCraft C# v2-dot-v3-context completion fix end'],
  ['// CodeCraft C# v2/v3 hybrid completion fix start', '// CodeCraft C# v2/v3 hybrid completion fix end'],
  ['// CodeCraft C# dot-v2 context-v3 completion fix start', '// CodeCraft C# dot-v2 context-v3 completion fix end'],
];
...
function patchSource(source) {
  ...
  let patched = source;
  for (const [startMarker, endMarker] of KNOWN_COMPLETION_HELPER_BLOCKS) {
    patched = stripBlock(patched, startMarker, endMarker);
  }
...
function validatePatchedSource(source) {
  const checks = [
    ...
    ['old v4 helper removed', !source.includes('// CodeCraft C# completion anywhere fix start') && !source.includes('2026-05-12-anywhere-v3')],
...
if (patched === original) {
  ...
  const backupPath = `${targetPath}.restore-v3-any-context-bak.${BACKUP_SUFFIX}`;
...
const reportPath = `${targetPath}.restore-v3-any-context-report.json`;
...
console.log('Next: run npm run build. Restored exact v3 marker: 2026-05-12-any-context-v3.');
