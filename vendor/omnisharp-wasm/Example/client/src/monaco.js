...
const iframeId = `omnisharp-${generateId()}`;
...
export class MonacoService {
  ...
  async initialize(monaco, iframeUrl = '/omnisharp/') {
    this.monaco = monaco;
    if (!this.monaco) {
    ...
    this.omnisharp = (method, ...args) => {
      ...
      return new Promise(res => {
        ...
        function handleMessage(event) {
          if (
            event.data?.omnisharp &&
            event.data.omnisharp.id === id &&
            !handled
          ) {
            window.removeEventListener('message', handleMessage);
            res(event.data.omnisharp.payload);
          }
        }
  ...
  async getDiagnosticsAsync(code) {
    const diagnostics = await this.omnisharp('GetDiagnosticsAsync', code);
    if (diagnostics) {
  ...
  async provideSignatureHelp(model, position) {
    ...
    try {
      const code = model.getValue();
      const res = await this.omnisharp('GetSignatureHelpAsync', code, req);
  ...
  async provideHover(_document, position) {
    const request = this._createRequest(position);
    try {
      const response = await this.omnisharp('GetQuickInfoAsync', request);
